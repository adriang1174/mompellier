    ...
    <div class="columnaIzq">
    	...
    	<?php 
            ...
            <div class="bloque formulario">
				...
				?>
           		<form action="/<?= $idEvento ?>/suscripcion" method="post" enctype="multipart/form-data" name="formInscribise" id="formInscribise">
				...
				<p>
                	...
                	<!-- <input id="fecnac" name="fecnac" type="text" class="campo" placeholder="dd/mm/aaaa"/> -->
                	<input id="dnac" name="dnac" type="text" class="campo_corto" placeholder="dd" maxlength="2"/>
                	<input id="mnac" name="mnac" type="text" class="campo_corto" placeholder="mm"  maxlength="2"/>
                	<input id="anac" name="anac" type="text" class="campo_corto" placeholder="aaaa"  maxlength="4"/>
                    <span>Año de graduación</span>
<!--                    <input id="fecgrad" name="fecgrad" type="text" class="campo" /> -->
                    <select id="fecgrad" name="fecgrad" class="campo" style="width:100px">
                    </select>
				</p>
                <p>
...
<!--                	<span>Condición</span>
				...
				<p>
                	 Como desea que nos contactemos con Ud.&#63;
				</p>
				<p>
                	<span>Lugar:</span><input id="lugar" name="lugar" type="text" class="campo" />
                    <span>Horario más conveniente:</span><input id="horario" name="horario" type="text" class="campo" />
				</p>
                ...
                <p style="text-align:right; padding:30px 23px; border-top: solid 1px #cccccc; margin-top:20px;"><a class="botonGenerico" style="padding:13px 22px" id="botonInscribirseVisita">INSCRIBIRSE ></a></p>
                </form>
            </div>
            ...
            <?php if ($_POST){
				...
				if (!$resp->is_valid) {
					...
					$("#fecgrad").val("'.$_POST["fecgrad"].'");
					$("#dnac").val("'.$_POST["dnac"].'");
					$("#mnac").val("'.$_POST["mnac"].'");
					$("#anac").val("'.$_POST["anac"].'");
					$("#lugar").val("'.$_POST["lugar"].'");
					$("#horario").val("'.$_POST["horario"].'");
					jAlert("<strong>El Captcha ingresado no es correcto.<br>Por favor vuelva a introducirlo.</strong>", "Alerta");
							...
							$fecnac			= mysqli_real_escape_string($link, $_REQUEST['dnac']).'/'.mysqli_real_escape_string($link, $_REQUEST['mnac']).'/'.mysqli_real_escape_string($link, $_REQUEST['anac']);
							$fecgrad		= mysqli_real_escape_string($link, $_REQUEST['fecgrad']);
							$lugar			= mysqli_real_escape_string($link, $_REQUEST['lugar']);
							$horario		= mysqli_real_escape_string($link, $_REQUEST['horario']);
							
							$idCurso = $_REQUEST['idEvento'];
							$sql = "insert into src_citas (Apellido,Nombre,Dni,especialidad,Hospital,Situacion,Telefono,Mail,Matricula,MatriculaProv,fechaNac,fechaGrad,lugar,horario) 
							values('{$apellido}','{$nombre}','{$dni}','{$especialidad}','{$hospital}','{$situacion}','{$telefono}','{$mail}','{$matricula}','{$matricula_prov}',STR_TO_DATE('{$fecnac}','%d/%m/%Y' ),'{$fecgrad}','{$lugar}','{$horario}')";
							var_dump($sql);
							...
							$mail->CharSet = "UTF-8";
							$mail->Subject = "Solicitud de cita de " . $_POST["nombre"] . " " . $_POST["apellido"];
							...
							<strong>Año de Graduación: </strong>" . $_POST["fecgrad"] . "<br>
							<strong>Lugar de preferencia para la vista: </strong>" . $_POST["lugar"] . "<br>
							<strong>Horario preferente: </strong>" . $_POST["horario"] . "<br>														
							</font>";
							if ($errorDB)
							{
							...
							$message = file_get_contents('http://www.comunidadresidentes.com.ar/mailing/visita/index.html'); 
			...
			<?php if ($accion=="gracias") {
					echo "<script>	populateEspec('');
								populateInst('');
								populateGrad('');
								jAlert('<strong>El Formulario ha sido enviado correctamente.</strong>', 'Muchas gracias por contactarnos');
					  </script>";
